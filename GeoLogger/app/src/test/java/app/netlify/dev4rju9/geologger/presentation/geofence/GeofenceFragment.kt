...
class GeofenceFragment : Fragment(R.layout.fragment_geofence), OnMapReadyCallback {
    ...
    private fun fetchGeofences () {
        lifecycleScope.launch {
            ...
            for (geofence in geofences) {
                ...
                addCircle(latLng, geofence.radius)
        ...
        val dialog = MaterialAlertDialogBuilder(requireActivity())
            ...
            .setPositiveButton("Save") { dialog, _ ->
                ...
                if (title.isNotEmpty() && radius.isNotEmpty()) {
                    addGeofence(title, latLng, radius.toFloat())
                    dialog.dismiss()
                } else {
                    Toast.makeText(
                        requireActivity(),
    ...
    private fun addGeofence(title: String, latLng: LatLng, radius: Float) {
        ...
        lifecycleScope.launch {
            geofenceViewModel.insertGeofence(
                GeofenceEntity(id = title, latitude = latLng.latitude, longitude = latLng.longitude, radius = radius)
            )
        }
